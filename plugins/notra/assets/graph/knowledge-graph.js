...
async function initialize() {
  ...
  state.graph = graph;
  state.governanceToken = await loadGovernanceSession();
...
function bindEvents() {
  ...
  elements.detailPanel.addEventListener("click", onDetailPanelClick);
  ...
  if (node.type === "practice" || node.type === "option") {
    return `
      ...
      >
        ...
        ${renderNodeLabelText(node, {
          className: "graph-node-label",
          x: 24,
          y: 50,
          maxWidth: node.width - 48,
          maxLines: 2,
          lineHeight: 16
        })}
  ...
  if (node.type === "project_profile") {
    return `
      ...
      >
        ...
        ${renderNodeLabelText(node, {
          className: "graph-node-label node-badge-label",
          x: 22,
          y: 42,
          maxWidth: node.width - 44,
          maxLines: 1,
          lineHeight: 15
        })}
...
function renderNodeLabelText(node, options) {
  const label = layoutNodeLabel(node.label || node.title || node.id, {
    maxWidth: options.maxWidth,
    maxLines: options.maxLines
  });
  const anchorAttribute = options.textAnchor ? ` text-anchor="${options.textAnchor}"` : "";
  const tspans = label.lines
    .map((line, index) => {
      const verticalAttribute =
        index === 0 ? `y="${options.y}"` : `dy="${options.lineHeight}"`;
      return `<tspan x="${options.x}" ${verticalAttribute}>${escapeHtml(line.text)}</tspan>`;
    })
    .join("");

  return `<text class="${options.className}"${anchorAttribute}>${tspans}</text>`;
}
...
function onDetailPanelClick(event) {
  const governanceButton = event.target.closest("[data-governance-action]");
  if (governanceButton) {
    handleGovernanceAction(governanceButton).catch((error) => {
      elements.graphSummary.textContent = `治理操作失败：${String(error.message || error)}`;
    });
    return;
  }

  onNodeReferenceClick(event);
}
...
async function handleGovernanceAction(button) {
  ...
  const response = await fetch("/api/governance/reject", {
    ...
    headers: {
      ...
      "x-notra-governance-token": state.governanceToken || ""
    },
    body: JSON.stringify({
      nodeId,
  ...
  if (!response.ok || !payload.ok) {
    throw new Error(payload.error || `治理 API 返回 ${response.status}`);
  }
...
function renderDetail(node, nodeMap) {
  ...
  elements.detailPanel.innerHTML = `
    ...
    ${renderDetailGovernanceBlock(node)}
  ...
  return `
    <section class="detail-block detail-evidence-block">
      ...
      <div class="detail-evidence-list">
        ${renderEvidenceItem(
          "source_path",
          renderSourcePathValue(node.source_path),
          "is-stacked"
        )}
        ${renderEvidenceItem(
          "source_evidence",
          renderEvidenceCollection(sourceEvidence),
          "is-stacked"
        )}
        ${renderEvidenceItem(
          "session_refs",
          renderEvidenceCollection(sessionRefs),
          "is-stacked"
        )}
      </div>
    </section>
...
function renderEvidenceItem(label, valueHtml, modifierClass = "") {
  return `
    <div class="detail-metric-item ${modifierClass}">
...
function renderSourcePathValue(sourcePath) {
  const fullPath = String(sourcePath || "").trim();
  if (!fullPath) {
    return `<span>无</span>`;
  }

  return `
    <div class="source-path" title="${escapeAttribute(fullPath)}">
      ${escapeHtml(shortenEvidencePath(fullPath, { segmentLimit: 6, maxLength: 84 }))}
    </div>
  `;
}

function renderEvidenceCollection(values) {
  const preview = buildEvidencePreview(values);
  if (preview.visibleItems.length === 0) {
    return `<span>无</span>`;
  }
  ...
  return `
    <div class="evidence-collection">
      <ul class="evidence-preview-list">
        ${preview.visibleItems.map((item) => renderEvidencePreviewItem(item)).join("")}
      </ul>
      ${
        preview.hiddenCount
          ? `
            <details class="evidence-more">
              <summary>还有 ${preview.hiddenCount} 项</summary>
              <ul class="evidence-preview-list is-secondary">
                ${preview.hiddenItems.map((item) => renderEvidencePreviewItem(item)).join("")}
              </ul>
            </details>
          `
          : ""
...
function renderEvidencePreviewItem(item) {
  if (item.pathLike) {
    return `
      <li class="evidence-preview-item is-path">
        <code class="evidence-path-text" title="${escapeAttribute(item.full)}">${escapeHtml(item.display)}</code>
      </li>
    `;
  }

  return `
    <li class="evidence-preview-item">
      <span class="evidence-text" title="${escapeAttribute(item.full)}">${escapeHtml(item.display)}</span>
    </li>
  `;
}
...
function markdownToHtml(markdown) {
  ...
  for (const line of lines) {
    ...
    if (isEvidenceSection(currentSection.title) && isPathLikeEvidence(trimmed)) {
  ...
  return sections
    .map(
      (section) => `
        <section class="${isEvidenceSection(section.title) ? "markdown-evidence-section" : ""}">
          <h5>${escapeHtml(section.title)}</h5>
          ${section.blocks.map((block) => renderMarkdownBlock(section.title, block)).join("")}
        </section>
      `
    )
    .join("");
...
function renderMarkdownBlock(sectionTitle, block) {
  if (block.type === "paragraph") {
    return `<p>${escapeHtml(block.text)}</p>`;
  }

  if (isEvidenceSection(sectionTitle)) {
    return renderEvidenceCollection(block.items);
  }

  return `<ul>${block.items.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>`;
}

function isEvidenceSection(title) {
  return String(title || "").trim().toLowerCase() === "evidence";
}
