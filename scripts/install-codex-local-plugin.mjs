...
const HOME_MARKETPLACE_NAME = "notra";
const HOME_MARKETPLACE_DISPLAY_NAME = "Notra";
const HOME_PLUGIN_NAME = "notra";
...
async function ensurePluginLink(sourcePluginPath, homePluginPath) {
  ...
  if (await exists(homePluginPath)) {
    ...
    throw new Error(
      `已存在其它 notra 插件目录，未自动覆盖: ${homePluginPath}\n当前指向: ${currentRealPath}`
    );
  }
...
function formatSummary(summary) {
  if (summary.action === "install") {
    return [
      ...
      "- 下一步: 完全重启 Codex，让 notra:notra-* 技能在启动时被原生发现。"
    ].join("\n");
  }
