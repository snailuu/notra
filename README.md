...
- 任务开始前用 `notra:notra-preflight` 查已有实践和推荐方案。
- 任务结束后用 `notra:notra-auto-crystallize` 自动记录采纳和孵化知识。
- 用采纳次数、分数和治理规则维护推荐池。
...
`.notra/` 是项目级知识库，也是 Markdown 事实源。图谱、索引、Obsidian 视图都是从这些 Markdown 文件生成出来的。
...
- 在 `~/plugins/notra` 创建指向当前仓库 `plugins/notra/` 的本地插件链接。
- 在 `~/.agents/plugins/marketplace.json` 写入 `notra` marketplace 条目。
...
`.notra/` 可以直接作为 Obsidian vault 打开。系统会维护：
...
├─ plugins/notra/                    # Codex / Claude Code 技能包
