...
test("resolveClaudePluginPaths uses the plugin directory convention", () => {
  ...
  assert.equal(
    ...
    path.join(repositoryRoot, "plugins", "notra")
  );
  assert.equal(
    paths.cachePluginPath,
    path.join(homeDir, ".claude", "plugins", "cache", "local", "notra", "0.1.16")
  );
  assert.equal(
  ...
  assert.equal(
    paths.oldSkillsPath,
    path.join(homeDir, ".claude", "skills", "notra")
  );
});

test("upsertInstalledPlugins creates a new payload when absent", () => {
  const installPath = path.resolve("/test/cache/notra");
  const next = upsertInstalledPlugins(null, installPath);

  assert.equal(next.version, 2);
  assert.ok(next.plugins["notra@local"]);
  assert.equal(next.plugins["notra@local"][0].scope, "user");
  assert.equal(next.plugins["notra@local"][0].installPath, installPath);
  assert.equal(next.plugins["notra@local"][0].version, "0.1.16");
  assert.ok(next.plugins["notra@local"][0].installedAt);
  assert.ok(next.plugins["notra@local"][0].lastUpdated);
});
...
test("upsertInstalledPlugins replaces an existing notra@local entry", () => {
  const existing = {
    version: 2,
    plugins: {
      "notra@local": [
  ...
  assert.equal(next.plugins["notra@local"].length, 1);
  assert.equal(next.plugins["notra@local"][0].installPath, installPath);
  assert.equal(next.plugins["notra@local"][0].version, "0.1.16");
});

test("removePluginFromInstalled removes notra@local and keeps others", () => {
  const existing = {
    version: 2,
    plugins: {
      "notra@local": [
...
test("upsertEnabledPlugin creates enabledPlugins when absent", () => {
  const next = upsertEnabledPlugin(null, "notra@local", true);

  assert.equal(next.enabledPlugins["notra@local"], true);
});
...
test("upsertEnabledPlugin preserves existing settings", () => {
  ...
  const next = upsertEnabledPlugin(existing, "notra@local", true);
...
test("removeEnabledPlugin removes notra@local and keeps others", () => {
  const existing = {
    env: { FOO: "bar" },
    enabledPlugins: {
      "notra@local": true,
  ...
  const next = removeEnabledPlugin(existing, "notra@local");

  assert.equal(next.env.FOO, "bar");
  assert.ok(!next.enabledPlugins["notra@local"]);
  assert.equal(next.enabledPlugins["other@marketplace"], true);
});

test("removeEnabledPlugin returns null when payload is absent", () => {
  const next = removeEnabledPlugin(null, "notra@local");
  assert.equal(next, null);
});

test("installClaudeLocalPlugin copies plugin to cache and writes installed_plugins.json and settings.json", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "notra-claude-test-"));
  const homeDir = path.join(tmpDir, "home");
  const repoRoot = path.join(tmpDir, "repo");

  const pluginPath = path.join(repoRoot, "plugins", "notra");
  await fs.mkdir(path.join(pluginPath, ".claude-plugin"), { recursive: true });
  await fs.writeFile(
    path.join(pluginPath, ".claude-plugin", "plugin.json"),
    JSON.stringify({ name: "notra", version: "0.1.16" }),
    "utf8"
  );
  await fs.mkdir(path.join(pluginPath, "skills", "notra-init"), { recursive: true });
  await fs.writeFile(
    path.join(pluginPath, "skills", "notra-init", "SKILL.md"),
    "---\nname: notra-init\n---\n# Notra Init\n",
    "utf8"
  );
  ...
  const cachePath = path.join(homeDir, ".claude", "plugins", "cache", "local", "notra", "0.1.16");
  assert.ok(await exists(cachePath));
  assert.ok(await exists(path.join(cachePath, ".claude-plugin", "plugin.json")));
  assert.ok(await exists(path.join(cachePath, "skills", "notra-init", "SKILL.md")));
  ...
  assert.ok(installed.plugins["notra@local"]);
  assert.equal(installed.plugins["notra@local"][0].scope, "user");
  assert.equal(installed.plugins["notra@local"][0].installPath, cachePath);
  assert.equal(installed.plugins["notra@local"][0].version, "0.1.16");

  const settingsPath = path.join(homeDir, ".claude", "settings.json");
  const settings = JSON.parse(await fs.readFile(settingsPath, "utf8"));

  assert.equal(settings.enabledPlugins["notra@local"], true);
...
test("installClaudeLocalPlugin cleans up old skills directory and stale cache", async () => {
  ...
  const oldSkillsPath = path.join(homeDir, ".claude", "skills", "notra");
  await fs.mkdir(oldSkillsPath, { recursive: true });
  await fs.writeFile(path.join(oldSkillsPath, "old.md"), "old", "utf8");

  const staleCachePath = path.join(homeDir, ".claude", "plugins", "cache", "local", "notra", "0.1.16");
...
test("uninstallClaudeLocalPlugin removes notra@local from installed_plugins and settings", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "notra-claude-test-"));
  const homeDir = path.join(tmpDir, "home");
  const repoRoot = path.join(tmpDir, "repo");
  ...
  const settings = JSON.parse(await fs.readFile(settingsPath, "utf8"));
  assert.ok(!settings.enabledPlugins["notra@local"]);
