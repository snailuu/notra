...
async function writePortablePreviewServer(knowledgeRoot, context) {
  await copyTreeSafe(path.join(previewRuntimeRoot, "core"), path.join(knowledgeRoot, portablePreviewRoot, "core"), context);
  await copyTreeSafe(path.join(previewRuntimeRoot, "server"), path.join(knowledgeRoot, portablePreviewRoot, "server"), context);
}
...
async function writeOpenGraphLauncher(knowledgeRoot, context) {
  ...
  const launcherSource = [
    ...
    "  exit /b 1",
    ")",
    'start "Notra Graph Server" powershell -NoExit -ExecutionPolicy Bypass -Command "& { & \\"%NODE_EXE%\\" \\"%SERVE_SCRIPT%\\" \\"%PROJECT_ROOT%\\" \\"%PORT%\\" }"',
