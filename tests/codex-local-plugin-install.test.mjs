...
test("resolveCodexHomeLocalPaths uses the home-local plugin convention", () => {
  ...
  assert.equal(paths.homePluginPath, path.join(homeDir, "plugins", "notra"));
  assert.equal(
...
test("upsertHomeMarketplace appends notra while preserving existing metadata", () => {
  const existing = {
  ...
  assert.equal(next.plugins[1].name, "notra");
  assert.equal(next.plugins[1].policy.installation, "INSTALLED_BY_DEFAULT");
});

test("upsertHomeMarketplace replaces an existing notra entry in place of stale config", () => {
  const existing = {
    name: "notra",
    interface: {
      displayName: "Notra"
