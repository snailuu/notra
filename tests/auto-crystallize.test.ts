...
test("autoCrystallizeSession skips project knowledge when notra-init has not run", async () => {
  ...
  assert.equal(result.knowledgeRoot, path.join(projectRoot, ".notra"));
...
test("autoCrystallizeSession records adopted recommendations when task matches existing knowledge", async () => {
  const projectRoot = await createInitializedSampleProject("project-knowledge-auto-hit-");
  ...
  const result = await autoCrystallizeSession(projectRoot, {
    ...
    // Phase 1 反馈闭环：agent 必须显式声明 adopted（option-unified-client 的 source_evidence
    // 包含 src/api/client.ts，因此与 touched_files 命中 → 计为 strong 信号）
  ...
  const session = await fs.readFile(
    path.join(projectRoot, ".notra", "sessions", "session-2026-04-28-auto-hit.md"),
    "utf8"
  );

  // strong 信号同步刷新 adopted_count / strong_count（fixture 起始 1，本轮 +1 = 2）
  assert.equal(usageIndex["option-unified-client"].adopted_count, 2);
  assert.equal(usageIndex["option-unified-client"].strong_count, 2);
...
test("autoCrystallizeSession creates incubating practice and option when no knowledge matches touched files", async () => {
  const projectRoot = await createInitializedSampleProject("project-knowledge-auto-miss-");
...
test("autoCrystallizeSession excludes volatile agent artifacts from touched files and evidence", async () => {
  const projectRoot = await createInitializedSampleProject("project-knowledge-auto-evidence-filter-");
...
test("autoCrystallizeSession avoids incubating knowledge when only volatile files changed", async () => {
  const projectRoot = await createInitializedSampleProject("project-knowledge-auto-only-volatile-");
...
test("auto crystallize CLI accepts JSON input and reports automatic metadata", async () => {
  const projectRoot = await createInitializedSampleProject("project-knowledge-auto-cli-");
  const inputPath = path.join(path.dirname(projectRoot), "auto-crystallize-input.json");
  await fs.writeFile(
    inputPath,
    `${JSON.stringify(
      {
